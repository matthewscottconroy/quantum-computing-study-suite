"""Summary screen for qec-trainer."""
...
class SummaryScreen(QWidget):
    ...
    def show_stats(self, stats: SessionStats) -> None:
        ...
        for cat, d in cat_stats.items():
            avg_s = sum(a.score for a in stats.attempts if a.problem.category == cat) / d["total"] if d["total"] else None
...
class _MiniCard(QFrame):
    def __init__(self, category: str, correct: int, total: int,
                 avg_score: float | None = None, parent=None) -> None:
        super().__init__(parent)
