...
class ProblemScreen(QWidget):
    answer_submitted = pyqtSignal(object, str, int, int)
    ...
    def _build_ui(self) -> None:
        ...
        mc_layout.setSpacing(8); mc_layout.setContentsMargins(0, 0, 0, 0)
        ...
        num_layout = QVBoxLayout(self._numeric_widget)
        num_layout.setContentsMargins(0, 0, 0, 0)
        num_lbl = QLabel("Enter your numeric answer:")
        num_lbl.setStyleSheet(f"font-size: 11px; font-weight: bold; color: {theme.TEXT_MUTED};")
        num_layout.addWidget(num_lbl)
        self._numeric_edit = QLineEdit()
        self._numeric_edit.setPlaceholderText("e.g. 0.4000")
        self._numeric_edit.textChanged.connect(self._validate_numeric)
        num_layout.addWidget(self._numeric_edit)
        self._numeric_widget.hide()
        root.addWidget(self._numeric_widget)
        ...
        self._text_edit.setPlaceholderText("Write your answer here…")
    ...
    def show_problem(self, problem: Problem, idx: int, total: int, streak: int = 0) -> None:
        ...
        elif problem.grade_mode == GradeMode.AUTO:
            self._numeric_widget.show()
            self._numeric_edit.clear()
    ...
    def _validate_numeric(self) -> None:
        self._submit_btn.setEnabled(bool(self._numeric_edit.text().strip()))
    ...
    def _on_submit(self) -> None:
        ...
        if self._problem.grade_mode == GradeMode.MC and self._problem.choices:
            answer = chr(65 + self._btn_group.checkedId())
        elif self._problem.grade_mode == GradeMode.AUTO:
            answer = self._numeric_edit.text().strip()
        else:
    ...
    def keyPressEvent(self, event) -> None:
        ...
        key = event.key()
        if self._problem and self._problem.grade_mode == GradeMode.MC and self._problem.choices:
            key_to_idx = {
