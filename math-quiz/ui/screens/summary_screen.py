...
class SummaryScreen(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self._chart_frame.setMinimumHeight(120)
    ...
    def _render_chart(self, stats: SessionStats) -> None:
        ...
        # Size frame to fit bars: 38px per subject + 50px for axis labels
        frame_h = max(120, len(subjects) * 38 + 50)
        self._chart_frame.setFixedHeight(frame_h)

        fig_h = frame_h / 96  # convert px → inches at 96 dpi
    ...
    def _on_export_json(self) -> None:
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Export session",
            f"math-quiz-{datetime.date.today()}.json", "JSON (*.json)"
        )
        if not path:
            return
        data = {
    ...
    def _on_export_markdown(self) -> None:
        ...
        path, _ = QFileDialog.getSaveFileName(
            ...
            f"math-quiz-{datetime.date.today()}.md", "Markdown (*.md)"
        )
        if not path:
            return
        lines: list[str] = []
        lines.append(f"# Math for Quantum — {datetime.date.today()}\n")
