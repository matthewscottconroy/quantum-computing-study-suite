"""MathSession — tracks config, stats, and SRS-weighted topic selection."""
...
class MathSession:
    def __init__(self, config: QuizConfig) -> None:
        ...
        # Shuffle-then-cycle ensures every question type appears once per
        # cycle before any type repeats — prevents e.g. 4 proof sketches in a row.
        ...
        # Load SRS weights once at session start rather than on every question.
        # avg_scores_by_subject/topic scan the whole history file; doing it
        # once here avoids repeated I/O during the session.
        try:
