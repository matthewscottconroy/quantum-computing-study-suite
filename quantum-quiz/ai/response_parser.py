...
def _strip_fences(raw: str) -> str:
    ...
    if text.startswith("```"):
        parts = text.split("```")
        # parts[0] is empty, parts[1] is the fenced block, strip language tag
        inner = parts[1]
...
def parse_question_response(raw: str) -> Question:
    data = _load(raw)
    try:
        # subject/topic/difficulty/question_type are injected by the worker
        # after parsing; the Claude response only provides text + hints.
        return Question(
            subject="",          # filled in by QuestionWorker
            topic="",
            difficulty="",
            question_type="",
